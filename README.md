...
- `facility_processors.py`  
  施設固有ロジック。現在は `sankoh` 向けの `宿泊施設` → `facility_code` 変換を実装。
- `db_importer.py`  
...
## Sankoh 固有の facility_code 変換

`config_db.yaml` の `a_and_c.facilities.sankoh` で `special_processor: sankoh` を指定すると、
